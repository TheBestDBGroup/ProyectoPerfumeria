...
const RealizarEvaluacionRenovacion = () => {
	...
	useEffect(() => {
  	...
  	const handleSelect = (id) => {
		history.replace(`/realizar-evaluacion/llenar-criterios/${id}/renovacion`);
	}
		...
		<Table>
			  ...
			  { proveedores.map( proveedor => (
			    <Table.Row>
			    	...
			    	<Table.Col>
			        	<Button onClick={() => handleSelect(proveedor.id_proveedor)} color="primary" className="eval-ren-buttons">Crear Nuevo</Button>
			      	</Table.Col>
			    </Table.Row>
			   ))}
