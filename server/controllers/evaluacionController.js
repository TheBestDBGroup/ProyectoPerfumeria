...
const getEscala = (request, response) => {
  ...
  const query =
    "SELECT e.min_escala, e.max_escala FROM ydm_escala e WHERE e.id_productor_escala = $1\
...
const getEvalCrits = (request, response) => {
  let values = [request.body.id_productor, request.body.tipo];

  const query =
    "SELECT ec.peso_prctj_eval_crit, ce.descripcion_criterio_eval, ce.tipo_criterio_eval\
...
const postFinalizarEvalCrits = (request, response) => {
  console.log("req.body", request.body);
  let valuesFinalizarEvalCrits = [
    request.body.id_productor,
    request.body.tipo_eval_crit,
  ];
  const queryFinalizarEvalCrits =
    "UPDATE ydm_eval_crit SET fecha_final_eval_crit = localtimestamp\
    WHERE tipo_eval_crit = $2 AND\
    id_productor_eval_crit IN (\
      SELECT ec.id_productor_eval_crit\
      FROM ydm_eval_crit ec\
      WHERE ec.id_productor_eval_crit = $1 AND ec.tipo_eval_crit= $2\
      AND ec.fecha_final_eval_crit is null\
      ORDER BY ec.id_criterio_eval_eval_crit, ec.fecha_inicial_eval_crit DESC) RETURNING *";
  pool.query(
    queryFinalizarEvalCrits,
    valuesFinalizarEvalCrits,
    (error, results) => {
      if (error) {
        throw error;
      }
...
const postCrearEvalCrit = (request, response) => {
  ...
  pool.query(queryCrearEvalCrit, valuesCrearEvalCrit, (error, results) => {
    if (error) {
      throw error;
    }
